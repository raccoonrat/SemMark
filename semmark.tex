...
随着大型语言模型（LLM）的广泛应用，AI生成内容的溯源和检测成为亟待解决的关键问题。文本水印技术通过在生成文本中嵌入不可感知的标记，为内容溯源和滥用检测提供了技术手段。与传统图像水印不同，文本水印面临语义保持、鲁棒性和检测效率等多重挑战。
...
\textbf{跨学科启示}：
\begin{itemize}[leftmargin=*]
\item \textbf{密码学方法借鉴}：密码学中的认证加密、零知识证明等概念可以为水印设计提供新的思路，如UPV的公开验证机制。
\item \textbf{信息论理论支撑}：信息论为水印容量上界和检测效率提供了理论基础，帮助理解水印系统的理论极限。
\item \textbf{机器学习安全类比}：对抗样本理论、模型逆向工程等概念可以启发新的攻击和防御方法。
\end{itemize}

\textbf{未来研究方向}：
\begin{itemize}[leftmargin=*]
\item \textbf{密码学方法应用}：研究如何将密码学中的先进方法（如零知识证明、同态加密）应用到水印设计中。
\item \textbf{信息论下界优化}：研究如何通过优化算法设计，接近信息论下界，提高水印系统的效率。
\item \textbf{机器学习安全融合}：研究如何将机器学习安全中的防御方法（如对抗训练）应用到水印系统中。
\end{itemize}
...
\textbf{第一阶段（2021--2022）}：\textbf{驱动因素}：大模型应用兴起，内容溯源需求凸显。\textbf{防御策略}：基础统计检验方法（如KGW的PRF分区）、token级水印。\textbf{攻击方法}：较少，主要关注基础攻击（如简单改写）。
...
\textbf{工程折中}：虽然在自然假设下强水印不可实现，但在现实威胁模型下，通过密钥管理、检测API限流/凭证化、跨语一致性增强等技术手段，仍可形成足够强且可部署的方案。
...
\textbf{白盒logits接入 vs 黑盒后处理}：黑盒后处理不需logits，第三方可施行，利于跨供应商部署；但插入词汇的语用痕迹与质量折衷需谨慎。

\textbf{单通道 vs 双通道}：单通道方法（概率或采样）通常在鲁棒性或质量上二选一；Duwak同时写入两路密纹并以对比搜索限制重复，显著降低检测样本量。

\textbf{有偏 vs 无偏}：无偏方法（Unbiased/DiPmark/MCMARK/STA-1）强调"不改变输出分布"，利于质量保持；但已有攻击/评测指出其在某些威胁模型下仍会出现可学性/可窃取性与多轮漂移。
...
\textbf{应用场景}：代码生成、API调用追踪、需求文档溯源
...
\item 安全等级：VERY HIGH（不可伪造性，满足高安全要求）
\end{itemize}
\item 合谋检测：
\begin{itemize}[leftmargin=*]
...
\item \textbf{溯源报告生成}：
\begin{itemize}[leftmargin=*]
\item 溯源报告生成：自动生成包含用户ID、时间戳、模型版本等信息的溯源报告
\item 报告格式：支持PDF、JSON等格式，便于技术分析和验证
\item 时间戳验证：支持时间戳的可信验证（基于区块链或可信时间戳服务）
\item 不可否认性：基于UPV的不可伪造性，保证溯源结果的不可否认性
\end{itemize}
\end{enumerate}
...
\item 用户隐私保护：用户ID编码需要考虑隐私保护，支持匿名化或加密编码
\item 数据安全：需要采用加密存储和传输，保护用户数据安全
\end{itemize}
...
\item 溯源服务：部署在安全环境中，负责生成和存储溯源报告
\end{itemize}
...
\item 隐私保护要求 → 考虑用户隐私保护（匿名化、加密编码）
\item 精确溯源要求 → 架构III（Multi-bit，支持精确溯源）
\end{itemize}
...
\item 软件开发：\$8--25K（水印嵌入、检测服务、溯源服务开发）
\item 测试与部署：\$2--5K（测试、部署、文档）
\end{itemize}
\item 运营成本：\$5/万次检测
\begin{itemize}[leftmargin=*]
\item 计算成本：\$4/万次（GPU、CPU、内存、网络、神经网络推理）
\item 存储成本：\$0.5/万次（日志、数据存储、溯源报告存储）
...
\textbf{架构I（KGW-based系统）}：
\begin{itemize}[leftmargin=*]
\item 技术收益：实时检测，低延迟，支持大规模部署
\item 成本节约：自动化检测，减少人工检测成本（节约\$50--100K/年）
...
\textbf{架构II（SemStamp系统）}：
\begin{itemize}[leftmargin=*]
\item 技术收益：高质量内容保护，跨语种检测，高鲁棒性
\item 成本节约：自动化质量评估，减少人工评估成本（节约\$100--200K/年）
...
\textbf{架构III（Multi-bit系统）}：
\begin{itemize}[leftmargin=*]
\item 技术收益：精确溯源，支持用户ID和时间戳编码，支持合谋检测
\item 成本节约：自动化溯源，减少人工溯源成本（节约\$200--500K/年）
...
\subsubsection{问题9$\star$: 硬件-算法协同的可能性边界}
...
\item \textbf{强水印不可能性理论不等于工程不可行}：虽然在自然假设下强水印不可实现，但在现实威胁模型下，通过密钥管理、检测API限流/凭证化、跨语一致性增强等技术手段，仍可形成足够强且可部署的方案。\textbf{工程可行标准}：在允许5\%误报率、检测成本$<$ \$10/万次检测的场景下，多比特水印可实现有效溯源（匹配率$\geq$95\%）。\textbf{成本效益分析}：Watermark Stealing攻击成本$<$ \$50，若防御方案的成本$>$ \$100，则工程价值有限；但在检测成本$<$ \$10的场景下，防御方案具有明显优势（成本效益比$>$10:1）。
\end{enumerate}
...
这些下界从根本上限制了可行水印的设计空间，使得某些"理想性质"无法同时实现。然而，这并不意味着工程不可行。在现实威胁模型下，通过密钥管理、检测API限流/凭证化、跨语一致性增强等技术手段，仍可形成足够强且可部署的方案。
...
\begin{itemize}[leftmargin=*]
\item \textbf{学术界}：应聚焦于理论下界的紧性分析、工程可行方案的探索，以及九个开放问题的解决
\item \textbf{工业界}：应根据应用场景选择合适的架构，在理论下界约束下实现工程可行性
\item \textbf{研究者}：应关注跨学科方法的应用，从密码学、信息论、机器学习安全等领域汲取新的思路
\end{itemize}
